...
IDs win each auction and log them. The data is not in sequence meaning that you will get recieve the bids in random time order. There are also some corrupt bid enteries which need to be filtered out and logged.
...
CSV as soon as it's initialized. Use the ```.data``` property from the data handler to recieve bid 
